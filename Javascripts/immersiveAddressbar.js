...
(function() {
	...
	const updater = {
		...
		scheduleUpdate(tab) {
			...
			const timeout = modState.setTimeout(() => {
	...
	async function initialize() {
		...
		modState.addChromeListener(chrome.tabs, 'onActivated', eventHandlers.onTabActivated);
		modState.addChromeListener(chrome.tabs, 'onUpdated', eventHandlers.onTabUpdated);
		modState.addChromeListener(chrome.tabs, 'onRemoved', eventHandlers.onTabRemoved);
		modState.addChromeListener(chrome.windows, 'onFocusChanged', eventHandlers.onWindowFocusChanged);

		const browser = dom.getBrowser();
		if (browser) {
			modState.addObserver(browser, eventHandlers.onThemeChanged, {
	...
	const startupCheckInterval = modState.setInterval(() => {
